...
nb_results <- lapply(split(da_long, da_long$celltype), function(df) {
...
ggplot(plot_df,
       aes(x = celltype, y = logFC, fill = Direction)) +
  ...
  labs(
    ...
    title = "Cell-type differential abundance (NB model)"
  )
...
ggplot(plot_df,
       aes(x = celltype, y = logFC_plot, fill = Direction)) +
...
ggplot(plot_df2,
  ...
  labs(
    ...
    y = "-log(FC)",
    fill = "Higher abundance in",
    title = "Celltype differential abundance by ctdna"
  )
...
ggplot(plot_df, aes(logFC, celltype)) +
